...
SCHEMAS = {"proof":"proof.schema.json","verificationKey":"verification-key.schema.json","issuer":"issuer.schema.json","revocation":"revocation.schema.json","evaluationRequest":"evaluation-request.schema.json","evaluationResult":"evaluation-result.schema.json","publicRecord":"public-record.schema.json","conformanceSuite":"conformance-suite.schema.json","conformanceReport":"conformance-report.schema.json","passport":"passport.schema.json","mandate":"mandate.schema.json","envelope":"transaction-envelope.schema.json","decision":"decision.schema.json","receipt":"receipt.schema.json","commerceOffer":"commerce/merchant-service-profile.schema.json","commerceMandate":"commerce/purchase-mandate.schema.json","commerceReceipt":"commerce/commerce-receipt.schema.json","rwaAsset":"rwa/asset-profile.schema.json","rwaStateClaim":"rwa/asset-state-claim.schema.json","rwaMandate":"rwa/asset-mandate.schema.json","rwaReceipt":"rwa/rwa-receipt.schema.json"}
...
def validate_schema(name: str, value: Any, schema_root: Path|None=None) -> list[str]:
    ...
    codes:set[str]=set(); _validate(value,schema,schema,codes,root)
    return sorted(codes)

def _validate(value:Any,schema:dict[str,Any],root:dict[str,Any],codes:set[str],schema_root:Path)->None:
    if "$ref" in schema:
        reference=schema["$ref"]
        if reference.startswith("#/"):
            target=root
            for part in reference.removeprefix("#/").split("/"): target=target[part]
            _validate(value,target,root,codes,schema_root)
        else:
            external=json.loads((schema_root/reference.rsplit("/",1)[-1]).read_text())
            _validate(value,external,external,codes,schema_root)
        return
    for branch in schema.get("allOf",[]): _validate(value,branch,root,codes,schema_root)
    ...
    if isinstance(value,list):
        ...
        if "items" in schema:
            for item in value: _validate(item,schema["items"],root,codes,schema_root)
    if isinstance(value,dict):
        ...
        for key,item in value.items():
            if key in properties: _validate(item,properties[key],root,codes,schema_root)
            elif isinstance(schema.get("additionalProperties"),dict): _validate(item,schema["additionalProperties"],root,codes,schema_root)
