...
python3 "$ROOT/sdk/python/conformance.py" --suite "$ROOT/conformance/suite-v0.1.json" --implementation-version 0.1.0-dev.0 --output "$ROOT/conformance/reports/python-sdk-0.1.0-dev.0.json"
(cd "$ROOT/sdk/go" && GOCACHE="${GOCACHE:-/tmp/oati-go-sdk-cache}" go run ./cmd/conformance --suite "$ROOT/conformance/suite-v0.1.json" --implementation-version 0.1.0-dev.0 --output "$ROOT/conformance/reports/go-sdk-0.1.0-dev.0.json")

node - "$ROOT" <<'NODE'
const fs = require("node:fs")
const root = process.argv[2]
const files = ["typescript-sdk-0.8.0-dev.0.json", "python-sdk-0.1.0-dev.0.json", "go-sdk-0.1.0-dev.0.json"]
